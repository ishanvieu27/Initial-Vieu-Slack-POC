...
// All Bolt listeners live here, attached to whatever App instance the caller
// hands in. src/app.js (local dev, Socket Mode) and api/slack/events.js
// (Vercel, HTTP mode) both construct their own App/receiver and call this —
// same handlers, two transports, zero duplication.
...
app.event('message', async ({ event, client }) => {
  ...
  if (isThreadReply) {
    ...
    if (!(await markSeen(eventKey))) return;
...
app.event('app_mention', async ({ event, client, context }) => {
  const eventKey = event.event_ts || event.ts;
  if (context && Number(context.retryNum) > 0) {
    console.log('[mention] skipping retry', eventKey);
    return;
  }
  if (!(await markSeen(eventKey))) {
    console.log('[mention] skipping duplicate', eventKey);
    return;
  }
